...
import { PEOPLE, type AppState, type Ingredient } from "@/lib/types";
import { lei } from "@/lib/format";

export default function AdminPanel({ data, pin, reload }: { data: AppState; pin: string; reload: () => void }) {
  const [draft, setDraft] = useState<Ingredient[]>(data.ingredients);
  const [msg, setMsg] = useState<string | null>(null);
  useEffect(() => setDraft(data.ingredients), [data.ingredients]);
  ...
  const rows = PEOPLE.map((p) => ({ name: p, order: data.orders[p] }));
  ...
  return (
    <div className="admin">
      <section aria-labelledby="ing-t">
        ...
        <p className="hint">Prețul e pe porție, pentru un sandviș. Ce bifezi ca sos devine capcană.</p>
        <ul className="editor">
          {draft.map((i) => (
